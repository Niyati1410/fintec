...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        setIsLoading(true);
        const response = await fetch("https://fintellectapi.onrender.com/visuals/sentiment_category"); 
        if (!response.ok) {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        <div className="text-center mb-12">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">Sentiment Analysis Dashboard</h1>
          <p className="text-lg text-gray-600">Visual insights from X</p>
        </div>
